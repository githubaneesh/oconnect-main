...
export class CoursesComponent implements OnInit {
  ...
  async getCourses()  {
        ...
        const response:any = await this.courseService.getCourses(this.chapter._id);
        if(response && !response.error){            
            let user = LocalStorageService.getInstance().getJSONObject('OConnect-UserInfo');
            ...
            const enrollIds :any = await this.courseService.getEnrolID(memberID,chapterID);
            for(let course of response)
            {
                ...
                console.log('description ', obj1.langstring['#text']);
     ...
     console.log('course   ', course);
       if(course.entity) {
        LocalStorageService.getInstance().saveData(`${course.entity._id}:course`, JSON.stringify(course.entity));
         this.router.navigateByUrl(`dashboard/member/course/${course.entity._id}/${course.enrollId}`)
        }
